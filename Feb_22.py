...
# =======================================Fault===================================================================
    if page =='Faults':
        ...
        df_daily = df2.loc[(df2['Cancel_Status'].isna()) & (df2['Work_Completed_Date'].notna()),:].copy() #some columns is not exist in df3, cos we drop some columns above
...
#--------------------------------------- color & width & opacity----------------------------------------------
    ...
    #--------------------------------------Daily Fault----------------------------------------------------------
     ...
     #---------------------------------------------KPI Monitoring-------------------------------------------------------
        ...
        x_KPI_building_3060_Responded = df3.groupby(by='KPI_For_Responded').Trade.value_counts().loc['30-120mins'].index
        x_KPI_building_60inf_Responded = df3.groupby(by='KPI_For_Responded').Trade.value_counts().loc['120-np.inf'].index

        y_KPI_building_010_Responded = df3.groupby(by='KPI_For_Responded', observed=True).Trade.value_counts().loc['0-10mins'].values
        y_KPI_building_1030_Responded = df3.groupby(by='KPI_For_Responded', observed=True).Trade.value_counts().loc['10-30mins'].values
        y_KPI_building_3060_Responded = df3.groupby(by='KPI_For_Responded', observed=True).Trade.value_counts().loc['30-120mins'].values
        y_KPI_building_60inf_Responded = df3.groupby(by='KPI_For_Responded', observed=True).Trade.value_counts().loc['120-np.inf'].values
        ...
        with fig_responded_building, _lock:
            fig_responded_building = go.Figure(data=[
                ...
                go.Bar(name='>120mins', x=x_KPI_building_60inf_Responded, y=y_KPI_building_60inf_Responded, marker_color=colorstackbarfail03)
            ])
        ...
        with fig_responded_category, _lock:
            fig_responded_category = go.Figure(data=[
                ...
                go.Bar(name='>120mins', x=x_KPI_category_60inf_Responded, y=y_KPI_category_60inf_Responded, marker_color=colorstackbarfail03),
            ])
            ...
            fig_responded_category.update_yaxes(title_text='Number of Fault', title_font_color=titlefontcolor, showgrid=True,
                                                ...
                                                linewidth=linewidth_xy_axis, linecolor=linecolor_xy_axis)
            fig_responded_category.update_layout(barmode='stack', title='KPI Monitoring(Responded) vs Trade Category', plot_bgcolor=plot_bgcolor)
        ...
        x_KPI_building_60inf_recovered = df3.groupby(by='KPI_For_Recovered').Trade.value_counts().loc['1-np.inf'].index
        ...
        y_KPI_building_60inf_recovered = df3.groupby(by='KPI_For_Recovered').Trade.value_counts().loc['1-np.inf'].values
...
#------------------------------------Number of fault vs people & wordcloud--------------------------------------
        ...
        with wc, _lock:
            ...
            stopwords = ['done', 'Done', 'check', 'Found', 'found', 'from', 'have',
                         'in', 'on', 'at', 'make', 'it', 'the', 'and', 'to', 'for', 'need', 'not', 'of', 'with', 
...
# =======================================================================================Inventories=========================================================================================
    if page =='Inventories':
        ...
        df_CO_t = pd.read_excel('Transaction_CO.xlsx', header=1, usecols=trans_cols, parse_dates=date)
        ...
        df_CO_balance = pd.read_excel('Inventories_CO.xlsx', header=1, usecols=balance_cols)
